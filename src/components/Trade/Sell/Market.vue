<template lang="pug">
  .limit
    ...
    .body
      span Price
      q-input(
        value="Sell at the best market price"
        dark
        dense
        :disable="true"
        outlined
...
export default {
  ...
  watch: {
    totalUpdateEvent () {
      this.total = (this.price * this.amount).toFixed(this.quoteToken.precision)
    },
  ...
  methods: {
    amountChange (e) {
      this.amount = parseFloat(e.target.value).toFixed(this.quoteToken.precision)
    },
    ...
    async buy () {
      ...
      try {
        const r = await transfer(
          ...
          `${(0).toFixed(this.baseToken.precision)} ${this.baseToken.symbol}@${this.baseToken.contract}`
        )
